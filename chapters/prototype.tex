...
So after we used the \textit{sudo rpi-update} command and rebooted the RPi2 we could continue.\newline
...
Since the UMTS stick is not the newest, it was not that easy to get it to work with the RPi2.
\subparagraph{Configure the USB config} \mbox{}\\
At first we had to configure the RPi2 so that it would accept the stick and accept it as a modem.\newline
...
\begin{minted}{python}
ACTION=="add", SUBSYSTEM=="usb", ATTRS{idVendor}=="0e8d", ATTRS{idProduct}=="0002", RUN+="/usr/sbin/usb_modeswitch -v 0e8d -p 0002 -M '555342431234567800000000000006f0010300000000000000000000000000'"
\end{minted}
Then we had to create a link to the right position, which was /opt/gps/udev/rules/ and we did that with the \textit{sudo ln -s /opt/gps/udev/rules/70-usb-modeswitch.rules} command.\newline
...
To configurations, we had to change in the super user state, with the \textit{sudo su} command.\newline
...
\end{minted}
After that we could start it with \textit{pon hot\_internet} and stop it with \textit{poff hot\_internet}.
