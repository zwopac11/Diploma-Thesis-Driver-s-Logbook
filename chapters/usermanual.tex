...
The green \gls{led} on the \gls{rpi2} lights up when the device boots and prepares itself for tracking. When it is ready and it receives data it flickers quick.\clearpageauthor \pageauthor{Laura Rössl}And if the red \gls{led} on the \gls{rpi2} \gls{gps} module blinks once per second, it has a \gls{gps} 
